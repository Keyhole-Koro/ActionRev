...
- `edges[].source` / `edges[].target` は `local_id` を参照する
- `edges[].local_id` は任意だが、ログ追跡と再試行時の差分比較のため付与を推奨する
- 永続化時にバックエンドが `node_id=nd_<ULID>`、`edge_id=ed_<ULID>` を採番する
...
| `edges[].local_id` | 任意 | 欠落しても保存処理は継続する |
...
- Pass 1 の保存時にバックエンドが `local_id -> node_id` の写像を確定し、edge はその写像で `source_node_id` / `target_node_id` を解決する
...
1. セクション単位の部分統合を行う
2. 各セクションの代表ノードと補完エッジだけを集めて最終統合を行う
...
- セクションごとに Pass 2-lite を実行し、重複統合・level 付与・局所エッジ整理を行う
- 各セクションから level 0〜2 の代表ノードとセクション間参照候補を抽出する
- 最終 Pass 2 では代表ノード群のみを対象に、文書全体の canonical 化と横断エッジ補完を行う
...
- Pass 2 完了後にバックエンドが surviving `local_id` ごとに `node_id` を採番し、edge の参照先を `source_node_id` / `target_node_id` に解決して永続化する
