...
- **多階層ノード**: `abstract` / `concrete` の2値ではなく、レベルとカテゴリで表現する
...
- `NodeType` の `abstract` / `concrete` は deprecated として残し、移行期間中は `level` と `category` で代替する
- 既存データには `level=1`（concept）/ `level=2`（concept）として再マッピングする
