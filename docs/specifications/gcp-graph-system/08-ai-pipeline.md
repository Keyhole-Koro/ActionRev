...
- `response_mime_type: "application/json"` + JSON Schema を指定してレスポンスを強制構造化する
- JSON Schema は proto の message 定義に対応させる

### リトライ戦略

- 最大リトライ回数: **3回**（初回呼び出しを含む）
- JSON parse 失敗もリトライ回数にカウントする
- バックオフ: 指数バックオフ（1s → 2s → 4s）
- 3回失敗した場合は `documents.status = "failed"` に更新してログに理由を記録する
- Gemini レート制限（429）は別カウントで最大5回リトライする（バックオフ: 10s → 20s → 40s → 80s → 160s）
