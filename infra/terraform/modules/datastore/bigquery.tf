...
locals {
  ds = google_bigquery_dataset.graph.dataset_id
}
...
resource "google_bigquery_table" "normalization_tools" {
  ...
  schema = jsonencode([
    ...
    { name = "description",       type = "STRING",  mode = "NULLABLE" },
    { name = "problem_pattern",   type = "STRING",  mode = "NULLABLE" },
    ...
    { name = "llm_review_score",  type = "FLOAT64", mode = "NULLABLE" },
...
resource "google_bigquery_table" "normalization_tool_runs" {
  ...
  schema = jsonencode([
    ...
    { name = "diff_summary",  type = "STRING",  mode = "NULLABLE" },
